...
class FDisk:
        ...
        try:
            ...
            if size > 0:
                if(self.unit == 'B'):
                    ...
                    printError("\t Fdisk>>> Ocurrio un error al definir el tamaño\n")
                    return False
            else:
                printError("\t Fdisk>>> El tamaño debe ser mayor a 0\n")
                return False
            ...
            printError("\t Fdisk>>> El tamaño debe ser un numero entero\n")
            return False
    
    def set_path(self, path): #Definir el path
        if(not os.path.exists(path)):
            printError("\t Fdisk>>> El path no existe\n")
            return False
        
        if(not path.endswith('.dsk')):
            printError("\t Fdisk>>> El path no tiene la extension .dsk\n")
            return False
        #Se guarda el path
        self.path = path
        return True
    ...
    def set_unit(self, unit): #Definir el unit
        if unit == None:
            ...
            return True
        printError("\t Fdisk>>> El unit no es valido\n")
        return False
    ...
    def set_type(self, type): #Definir el type
        if type == None:
            ...
            return True
        printError("\t Fdisk>>> El type no es valido\n")
        return False
    ...
    def set_fit(self, fit): #Definir el fit
        if fit == None:
            ...
            return True
        printError("\t Fdisk>>> El fit no es valido\n")
        return False
    ...
    def set_delete(self, delete): #Definir el delete
        if delete == None:
            ...
            return True
        printError("\t Fdisk>>> El valor del delete no es valido\n")
        return False
    ...
    def set_add(self, add): #Definir el add
        try:
            ...
            printError("\t Fdisk>>> El valor del add no es valido\n")
            return False
    ...
    def run(self, size, path, name, unit, type, fit, delete, add):
        if(delete != None and add != None):
            printError("\t Fdisk>>> No se puede modificar el tamaño y eliminar una particion a la vez\n")
            return False
        ...
        if(self.set_delete(delete)):
            ...
            if(self.delete_partitions()):
                printText("\t Fdisk>> Se elimino la particion\n")
                return True
            return False
            ...
            if(self.change_space()):
                printText("\t Fdisk>>> Se cambio el tamaño de la particion\n")
                return True
            return False
        else:
            if(not self.set_unit(unit)): return False
        ...
        if(self.create_partition()):
            printText("\t Fdisk>>> Se creo la particion\n")
            return True
        return False
    ...
    def create_partition(self):
        try:
            ...
            if(crr_mbr.exist_partition(self.name)):
                printError("\t Fdisk>>> Ya existe una particion con ese nombre\n")
                return False
            ...
            if(self.type == 'E'):
                for partition in crr_mbr.mbr_partition:
                    if(partition.part_type == b'E'):
                        printError("\t Fdisk>>> Ya existe una particion extendida\n")
                        return False
            ...
            if(self.type == 'L'):
                ...
                for partition in crr_mbr.mbr_partition:
                    if(partition.part_type == b'E'):
                        ...
                        break
                if(not existE):
                    printError("\t Fdisk>>> No existe una particion extendida para crear una particion logica\n")
                    return False
                ...
                if(aux_ebr.part_status == b'\0'):
                    if(self.size > partitionextendida.part_size):
                        printError("\t Fdisk>>> No hay espacio suficiente para crear una particion logica\n")
                        return False
                    nuevo_ebr = Ebr()
                    nuevo_ebr.set_ebr('1', self.fit, startp, self.size, -1, self.name)
                ...
                #Dependiendo del fit
                ebr_temp = Ebr()                
                diferencias = ebr_temp.get_freespace(lista_ebr, partitionextendida.part_size + partitionextendida.part_start)
                indexplogica = 0
                espacioentrep = False
                ...
                if(indexplogica == len(lista_ebr)-1):
                    nuevo_ebr.set_ebr('1', self.fit, aux_ebr.part_start + aux_ebr.part_size, self.size, -1, self.name)
            ...
            if(indexp > 3):
                printError("\t Fdisk>>> No se pueden crear mas particiones\n")
                return False
            
            #Se verifica que el tamaño de la particion sea menor al tamaño del disco
            if(size_particiones + self.size > crr_mbr.mbr_tamano):
                printError("\t Fdisk>>> No queda espacio suficiente en el disco para almacenar la particion\n")
                return False
            ...
            printError(f"\t Fdisk>>> Error al crear la particion:{e}\n")
            return False
    ...
    def delete_partitions(self):
        ...
        if(not existeP):
            if(not existePext):
                printError("\t Fdisk>>> No existen coincidencias con el nombre de la particion\n")
                return False
            ...
            for cadaebr in lista_ebr:
                if cadaebr.part_name.decode() == self.name:
                    if(indexp_logic == 0):
                        printError("\t Fdisk>>> No se puede eliminar el ebr inicial\n")
                        return False
                    
                    #Mensaje de confirmacion
                    confirm = inputWarning("\t Fdisk>>> Esta seguro que desea eliminar la particion? (Y/N): ")
            ...
            for cadaebr in lista_ebr:
                if cadaebr.part_name.decode() == self.name:
                    ...
                    if(cadaebr.part_size + self.add <= 0):
                        printError("\t Fdisk>>> No Hay sufienciente espacio para reducir la particion\n")
                        return False
                    ...
                    if(indexp_logic < len(lista_ebr)-1):
                        ...
                        if(cadaebr.part_start + cadaebr.part_size + self.add > particion_siguiente.part_start):
                            printError("\t Fdisk>>> No se puede agregar espacio a la particion\n")
                            return False
        ...
        if(particion_cambiar != None):
            if(particion_cambiar.part_type.decode() != 'L'):
                if(particion_cambiar.part_size + self.add < 0):
                    printError("\t Fdisk>>> No se puede reducir la particion\n")
                    return False
                ...
                particion_nuevosize.set_partition('1', particion_cambiar.part_type.decode(), particion_cambiar.part_fit.decode(), particion_cambiar.part_start, particion_cambiar.part_size + self.add, particion_cambiar.part_name.decode())
